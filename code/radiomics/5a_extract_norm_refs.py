# File: 5a_norm_refs.py
# Date: 07/15/2024
# Author: Lawrence Chillrud <chili@u.northwestern.edu>
# Description: Performs radiomics feature extraction on the preprocessed MRI images using their segmentations.
#
# For each subject in the cohort, this script extracts radiomics features from the preprocessed MRI images using the complement of their accompanying segmentations.
# See the description of the extract_features function for more details on the extraction process.
#
# This script relies on the following file(s) as inputs:
#   * data/preprocessing/output/7c_NONLIN_WARP_COMPLETED_PREPROCESSED/*
#   * data/8_mni_registered_mixed_segs/*
#
...
# instead of EXTRACTOR.enableAllFeatures(), we enable the feature classes by hand, to exclude shape based features, since there is no need to normalize those...
    ...
    * Saves the extracted features to the OUTPUT_FILE csv file.
    ...
    for scan, sequence in tqdm(zip(scan_paths, scan_sequences), total=len(scan_paths), dynamic_ncols=True, position=1, desc=f'Scans from subject {sub_no}', leave=False, colour='blue'):
        try:
            ...
            features_row = pd.concat([pd.DataFrame({'Subject Number': [sub_no], 'Segmentation Label': [0], 'Scan Sequence': [sequence]}), pd.Series(result_trimmed).to_frame().T], axis=1)
            if not os.path.exists(OUTPUT_FILE):
                features_row.to_csv(OUTPUT_FILE, index=False)
            else:
                features_row.to_csv(OUTPUT_FILE, mode='a', index=False, header=False)
        
            logging.info(f"[Subject number: {sub_no}, Annotation label: 0, Sequence: {sequence}] was a SUCCESS!\n")
        except Exception as e:
            logging.info(f"[Seg subject: {sub_no}, Annotation label: 0, Sequence: {sequence}] FAILED and had an ERROR: {e}\n")
            continue
