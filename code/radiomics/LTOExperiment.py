...
class LTOExperiment:
    ...
    def model_fit_predict(self, train_idx, test_idx, lmda):
        ...
        if self.n_classes == 2:
            train_metrics = self.get_binary_metrics(train_probs, train_preds, y_train[:len(self.X)-to_sub])
        else:
            train_metrics = self.get_multiclass_metrics(train_probs, train_preds, label_binarize(y_train[:len(self.X)-to_sub], classes=np.arange(self.n_classes)))
    ...
    def inner_loop(self, train_val_idx, test_idx, lmda):
        ...
        if self.n_classes == 2:
            val_metrics = self.get_binary_metrics(val_probs, val_preds, y_val)
        else:
            val_metrics = self.get_multiclass_metrics(val_probs, val_preds, label_binarize(y_val, classes=np.arange(self.n_classes)))
    ...
    def process_lambda(self, lmda):
        ...
        with ProcessPoolExecutor(max_workers=32) as executor:
            ...
            futures = [executor.submit(self.inner_loop, train_val_idx, test_idx, lmda)
                       for train_val_idx, test_idx in LeaveOneOut().split(self.X)]
    ...
    def par_lto_model(self, pmetric='AUC'):
        ...
        test_perf_met_by_lambda = self.plot_metric_by_lambda(train_metrics_by_lambda, val_metrics_by_lambda, test_metrics_by_lambda, pmetric)
        for metric in test_metrics_by_lambda[self.lambdas[0]].columns:
            self.plot_metric_by_lambda(train_metrics_by_lambda, val_metrics_by_lambda, test_metrics_by_lambda, metric)
