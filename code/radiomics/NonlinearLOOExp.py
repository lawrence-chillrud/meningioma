...
class NonlinearLOOExp:
    def __init__(self, model, fixed_model_params, param_to_sweep, sweep_values, prediction_task, use_smote=True, scaler='Standard', seed=0, output_dir='data/radiomics_loo', save=True, feat_file="data/radiomics/features6/features_wide.csv", feat_select=None):
        ...
        * use_smote: Whether to use SMOTE for oversampling the minority class. If True, the scaler must be provided. Only makes sense for imbalanced datasets (i.e., NOT MethylationSubgroup).
        ...
        self.param_to_sweep = param_to_sweep
        ...
        # Set up logging
        logging.basicConfig(filename=f'{self.output_dir}/exp.log', level=logging.INFO, format='%(message)s')
        bar = '-'*80
        logging.info(f'\n{bar}\nStarting Nonlinear LOO Experiment for {self.prediction_task}\n{bar}\n')
        logging.info(f'Model: {self.model.__name__}')
        logging.info(f'Fixed Model Parameters: {self.fixed_model_params}')
        logging.info(f'Parameter to Sweep: {self.param_to_sweep}')
        logging.info(f'Sweep Values: {self.sweep_values}')
        logging.info(f'Use SMOTE: {self.use_smote}')
        logging.info(f'Scaler: {self.scaler}')
    ...
    def plot_metric_by_setting(self, train_metrics_by_setting, test_metrics_by_setting, metric):
        # Processing Training Data
        train_means = {}
        train_stds = {}
        for key, df in train_metrics_by_setting.items():
            if metric in df.columns:
                train_means[key] = df[metric].mean()
                train_stds[key] = df[metric].std()
            else:
                print(f"Metric {metric} not found in training data for setting {key}")

        # Converting dictionaries to DataFrames
        train_mean_df = pd.DataFrame(list(train_means.items()), columns=['Setting', 'Mean'])
        train_std_df = pd.DataFrame(list(train_stds.items()), columns=['Setting', 'Std Dev'])

        # Processing Test Data
        test_metrics = {k: df[metric].iloc[0] for k, df in test_metrics_by_setting.items() if metric in df.columns}
        test_df = pd.DataFrame(list(test_metrics.items()), columns=['Setting', metric])

        # Pick best Setting and it's value to include in title
        max_perf_met_index = test_df[metric].idxmax()

        best_Setting = test_df.loc[max_perf_met_index, 'Setting']
        # if best_Setting is a float, round it to 3 decimal places
        if isinstance(best_Setting, float): best_Setting = round(best_Setting, 3)
        best_value = round(test_df.loc[max_perf_met_index, metric], 3)

        # sort dfs by Setting value
        train_mean_df = train_mean_df.sort_values(by='Setting')
        train_std_df = train_std_df.sort_values(by='Setting')
        test_df = test_df.sort_values(by='Setting')

        plt.figure(figsize=(12, 8))

        # Plotting Training Data
        plt.errorbar(train_mean_df['Setting'], train_mean_df['Mean'], yerr=train_std_df['Std Dev'], label=f'Train {metric}', marker='o')

        # Plotting Test Data
        plt.plot(test_df['Setting'], test_df[metric], label=f'Test {metric}', linestyle='--', marker='x')

        plt.xlabel('Setting')
        plt.ylabel(metric)
        plt.title(f'{self.prediction_task}: Classic LOO CV Evolution Curve\nBest Setting = {best_Setting} by {metric} = {best_value}')
        plt.legend()
        plt.grid(True)
        if self.save:
    ...
    def model_fit_predict(self, train_idx, test_idx, lmda_val):
        ...
        current_model_params = self.fixed_model_params.copy()
        current_model_params[self.param_to_sweep] = lmda_val
        model = self.model(**current_model_params)
        ...
        return (test_probs, test_preds, y_test, train_metrics)

    def process_setting(self, lmda_val):
        results = []

        # Create a ProcessPoolExecutor to handle tasks in parallel (4 max workers)
        with ProcessPoolExecutor(max_workers=1) as executor:
            # Create tasks for each train-test split
            futures = [executor.submit(self.model_fit_predict, train_idx, test_idx, lmda_val)
                       for train_idx, test_idx in LeaveOneOut().split(self.X)]

            # Use tqdm to monitor progress
            for future in tqdm(as_completed(futures), total=len(futures), desc=f"Processing {self.param_to_sweep}={lmda_val}"):
                results.append(future.result())

        # Unpack results
        test_probs, test_preds, y_test, train_metrics = zip(*results)
    ...
    def par_loo_model(self, pmetric='AUC'):
        ...
        # Use ProcessPoolExecutor to parallelize the loop over lambdas (8 max workers)
        with ProcessPoolExecutor(max_workers=1) as executor:
            futures = {executor.submit(self.process_setting, lmda_val): lmda_val for lmda_val in self.sweep_values}
            for future in as_completed(futures):
                result = future.result()
                setting = result['setting']
                test_probs_by_lambda[setting] = result['test_probs']
                test_preds = result['test_preds']
                y_tests_by_lambda[setting] = result['y_test']
                train_metrics_by_lambda[setting] = result['train_metrics']
                if self.n_classes == 2:
                ...
                test_metrics_by_lambda[setting] = test_metrics
        
        self.perf_metric = pmetric
        test_perf_met_by_lambda = self.plot_metric_by_setting(train_metrics_by_lambda, test_metrics_by_lambda, pmetric)
        for metric in test_metrics_by_lambda[self.sweep_values[0]].columns:
            self.plot_metric_by_setting(train_metrics_by_lambda, test_metrics_by_lambda, metric)

        max_perf_met_index = test_perf_met_by_lambda[pmetric].idxmax()
        self.best_lambda = test_perf_met_by_lambda.loc[max_perf_met_index, 'Setting']
        if isinstance(self.best_lambda, float): self.best_lambda = round(self.best_lambda, 2)
