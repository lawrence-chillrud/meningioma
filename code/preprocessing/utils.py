...
def view_intensity_standardization(data_dir='data/preprocessing/output', scan_type='AX_3D_T1_POST', num_subjects=4, cmap='gray'):
    ...
    fig, axs = plt.subplots(2, num_subjects + 1, figsize=(6*(num_subjects + 1), 6))
    fig.suptitle(f'{scan_type}: Intensity Standardization Before vs. After', fontsize=24, y=1.05)
    for i, subject in enumerate(subjects_to_plot):
      session = f'{subject}_Brainlab'
      scans = lsdir(f'{after_dir}/{subject}/{session}')
      # print("Scans: ", scans)
      ...
      axs[0, i].set_title(f'{subject} Before', fontsize=15)
      axs[0, i].set_xticks([])
      axs[0, i].set_yticks([])
      ...
      axs[1, i].set_title(f'{subject} After', fontsize=15)
      axs[1, i].set_xticks([])
      axs[1, i].set_yticks([])
