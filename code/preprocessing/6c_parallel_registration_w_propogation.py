...
def register_subject(subject):
    ...
    for session in lsdir(f'{data_dir}/{subject}'):
        ...
        if not has_post:
            logging.info(f"Warning: No AX_3D_T1_POST scan found for {session}, therefore skipping session: {session}")
            continue
        else:
        ...
        if ax3dt1post.spacing != (1.0, 1.0, 1.0):
            logging.info(f"\tWarning: {session}/{ax3dt1post_path} does not have 1x1x1mm spacing. Instead it has: {ax3dt1post.spacing}")
        
        swi_intermediary_transform = None
        dwi_intermediary_transform = None

        type_of_transform = 'Rigid'
        logging.info(f"\tPerforming {type_of_transform.lower()} registration to {rigid_template_path.split('/')[-1]} for scan {nonlinear_template}")
        ...
        logging.info(f"\tNonlinear warping the pre-requisite scans {pre_req_scans} to {ax3dt1post_path}, then propogating rigid registration ({ax3dt1post_path} -> {rigid_template_path.split('/')[-1]}) onto each scan")
        for scan in pre_req_scans:
            ...
            if not os.path.exists(f'{cur_output_dir}/{session}_{scan}.nii.gz'): 
                ...
                try:
                    logging.info(f"\t\t\tNonlinear warping {scan} to {ax3dt1post_path}")
                    ...
                    logging.info(f"\t\t\tPropogating the rigid registration from {ax3dt1post_path} -> {rigid_template_path.split('/')[-1]} onto {scan}")
                    ...
                    logging.info(f"\t\t\tFixed shape: {ax3dt1post.numpy().shape}")
        ...
        if len(swi_dwi_scans) > 0:
            logging.info(f"\tNonlinear warping {swi_dwi_scans} scans to {ax3dt1post_path} then propogating rigid registration ({ax3dt1post_path} -> {rigid_template_path.split('/')[-1]}) onto each scan")
            for scan in swi_dwi_scans:
                ...
                if not os.path.exists(f'{cur_output_dir}/{session}_{scan}.nii.gz'): 
                    ...
                    logging.info(f"\t\t\tSubstep 1/3: Nonlinear warping {scan} to {intermediary_path} (from {intermediary_dir})")
                    ...
                    try:
                        ...
                        logging.info(f"\t\t\tSubstep 2/3: Propogating the nonlinear warp from {intermediary_path} -> {ax3dt1post_path} onto {scan}")
                        ...
                        logging.info(f"\t\t\tSubstep 3/3: Propogating the rigid registration from {ax3dt1post_path} -> {rigid_template_path.split('/')[-1]} onto {scan}")
                        propogated_rigid_transform = ants.apply_transforms(
                            fixed=mni_template,
