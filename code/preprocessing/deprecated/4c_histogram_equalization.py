...
for i, scan_type in enumerate(scan_types_to_normalize):
    ...
    for image, output_fp, aff_mat in tqdm(zip(arr_images, cur_output_filepaths, affine_mats), desc=f"Equalizing and saving {scan_type} images", leave=True, total=len(cur_output_filepaths)):
        ...
        # or z score norm w/brain mask # find mean and std dev across all patients for just adc
        # exposure.equalize_adapthist
        equalized_im = exposure.equalize_hist(im, mask=im > im.mean()) # or exposure.equalize_adapthist(image, kernel_size=???, clip_limit=???) # larger kernel size than default ! larger kernel = smoother image
